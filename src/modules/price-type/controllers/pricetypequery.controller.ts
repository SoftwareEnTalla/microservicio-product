...
import { PriceTypeDto } from "../dtos/all-dto";
...
@ApiTags("PriceType Query")
@UseGuards(PriceTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("pricetypes/query")
export class PriceTypeQueryController {
  #logger = new Logger(PriceTypeQueryController.name);

  constructor(private readonly service: PriceTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all pricetype with optional pagination" })
  @ApiResponse({ status: 200, type: PriceTypesResponse })
  @ApiQuery({ name: "options", required: false, type: PriceTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PriceType>    
  ): Promise<PriceTypesResponse<PriceType>> {
    try {
     
      const pricetypes = await this.service.findAll(options);
      logger.info("Retrieving all pricetype");
      return pricetypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find pricetype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter pricetype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PriceTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PriceType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find pricetypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PriceType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all pricetypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count pricetypes with conditions" })
  @ApiResponse({ status: 200, type: PriceTypesResponse<PriceType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PriceTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one pricetype with conditions" })
  ...
  @ApiOperation({ summary: "Find one pricetype or return error" })
  @ApiResponse({ status: 200, type: PriceTypeResponse<PriceType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PriceType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get pricetype by ID" })
  @ApiResponse({ status: 200, type: PriceTypeResponse<PriceType> })
  @ApiResponse({ status: 404, description: "PriceType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the pricetype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PriceTypeResponse<PriceType>> {
    try {
      const pricetype = await this.service.findOne({ where: { id } });
      if (!pricetype) {
        throw new NotFoundException(
          "PriceType no encontrado para el id solicitado"
        );
      }
      return pricetype;
    } catch (error) {
