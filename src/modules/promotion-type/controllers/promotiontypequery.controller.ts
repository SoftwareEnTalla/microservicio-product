...
@ApiTags("PromotionType Query")
@UseGuards(PromotionTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("promotiontypes/query")
export class PromotionTypeQueryController {
  #logger = new Logger(PromotionTypeQueryController.name);

  constructor(private readonly service: PromotionTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all promotiontype with optional pagination" })
  @ApiResponse({ status: 200, type: PromotionTypesResponse })
  @ApiQuery({ name: "options", required: false, type: PromotionTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PromotionType>    
  ): Promise<PromotionTypesResponse<PromotionType>> {
    try {
     
      const promotiontypes = await this.service.findAll(options);
      logger.info("Retrieving all promotiontype");
  ...
  @ApiOperation({ summary: "Find promotiontype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter promotiontype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PromotionTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PromotionType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find promotiontypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PromotionType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all promotiontypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count promotiontypes with conditions" })
  @ApiResponse({ status: 200, type: PromotionTypesResponse<PromotionType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PromotionTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one promotiontype with conditions" })
  ...
  @ApiOperation({ summary: "Find one promotiontype or return error" })
  @ApiResponse({ status: 200, type: PromotionTypeResponse<PromotionType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PromotionType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get promotiontype by ID" })
  @ApiResponse({ status: 200, type: PromotionTypeResponse<PromotionType> })
  @ApiResponse({ status: 404, description: "PromotionType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the promotiontype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PromotionTypeResponse<PromotionType>> {
    try {
      const promotiontype = await this.service.findOne({ where: { id } });
      if (!promotiontype) {
        throw new NotFoundException(
          "PromotionType no encontrado para el id solicitado"
        );
      }
