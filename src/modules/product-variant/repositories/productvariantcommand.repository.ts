...
export class ProductVariantCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: ProductVariant): Promise<ProductVariant> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publish(new ProductVariantCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: ProductVariant[]): Promise<ProductVariant[]> {
    ...
    if (this.shouldPublishEvent()) {
      this.eventPublisher.publishAll(result.map((el)=>new ProductVariantCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      this.eventPublisher.publish(new ProductVariantUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            this.eventPublisher.publish(new ProductVariantUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       this.eventPublisher.publish(new ProductVariantDeletedEvent(id, {
        instance: entity,
        metadata: {
