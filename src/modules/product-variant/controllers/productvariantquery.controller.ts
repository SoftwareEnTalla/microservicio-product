...
@ApiTags("ProductVariant Query")
@Controller("productvariants/query")
export class ProductVariantQueryController {
  #logger = new Logger(ProductVariantQueryController.name);

  constructor(private readonly service: ProductVariantQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all productvariant with optional pagination" })
  @ApiResponse({ status: 200, type: ProductVariantsResponse })
  @ApiQuery({ name: "options", required: false, type: ProductVariantDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProductVariant>    
  ): Promise<ProductVariantsResponse<ProductVariant>> {
    try {
     
      const productvariants = await this.service.findAll(options);
      logger.info("Retrieving all productvariant");
  ...
  @ApiResponse({ status: 404, description: "ProductVariant not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the productvariant to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProductVariantResponse<ProductVariant>> {
    try {
      const productvariant = await this.service.findOne({ where: { id } });
      if (!productvariant) {
  ...
  @ApiOperation({ summary: "Find productvariant by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter productvariant', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProductVariantsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProductVariant no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find productvariants with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProductVariant>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all productvariants" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count productvariants with conditions" })
  @ApiResponse({ status: 200, type: ProductVariantsResponse<ProductVariant> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one productvariant with conditions" })
  @ApiResponse({ status: 200, type: ProductVariantResponse<ProductVariant> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one productvariant or return error" })
  @ApiResponse({ status: 200, type: ProductVariantResponse<ProductVariant> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
