...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class ProductPromotionQueryRepository {

    //Constructor del repositorio de datos: ProductPromotionQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination ProductPromotion on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[ProductPromotion[], number]> {
      logger.info('Ready to findAndCount ProductPromotion on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of ProductPromotion on repository:',result);
      return result;
    }
