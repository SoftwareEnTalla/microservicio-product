...
import {
  ProductPriceCreatedEvent,
  ProductPriceUpdatedEvent,
  ProductPriceDeletedEvent,

} from '../events/exporting.event';
import {
  SagaProductPriceFailedEvent
} from '../events/productprice-failed.event';
import {
...
export class ProductPriceCrudSaga {
  ...
  @Saga()
  onProductPriceCreated = ($events: Observable<ProductPriceCreatedEvent>) => {
    return $events.pipe(
  ...
  @Saga()
  onProductPriceUpdated = ($events: Observable<ProductPriceUpdatedEvent>) => {
    return $events.pipe(
  ...
  @Saga()
  onProductPriceDeleted = ($events: Observable<ProductPriceDeletedEvent>) => {
    return $events.pipe(
  ...
  private handleSagaError(error: Error, event: any) {
    ...
    this.eventBus.publish(new SagaProductPriceFailedEvent( error,event));
  }
}
