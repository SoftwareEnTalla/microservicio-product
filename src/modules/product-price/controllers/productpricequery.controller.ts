...
import { ProductPriceDto } from "../dtos/all-dto";
...
@ApiTags("ProductPrice Query")
@Controller("productprices/query")
export class ProductPriceQueryController {
  #logger = new Logger(ProductPriceQueryController.name);

  constructor(private readonly service: ProductPriceQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all productprice with optional pagination" })
  @ApiResponse({ status: 200, type: ProductPricesResponse })
  @ApiQuery({ name: "options", required: false, type: ProductPriceDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProductPrice>    
  ): Promise<ProductPricesResponse<ProductPrice>> {
    try {
     
      const productprices = await this.service.findAll(options);
      logger.info("Retrieving all productprice");
  ...
  @ApiOperation({ summary: "Find productprice by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter productprice', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProductPricesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProductPrice no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find productprices with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProductPrice>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all productprices" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count productprices with conditions" })
  @ApiResponse({ status: 200, type: ProductPricesResponse<ProductPrice> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ProductPrices no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one productprice with conditions" })
  ...
  @ApiOperation({ summary: "Find one productprice or return error" })
  @ApiResponse({ status: 200, type: ProductPriceResponse<ProductPrice> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ProductPrice no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get productprice by ID" })
  @ApiResponse({ status: 200, type: ProductPriceResponse<ProductPrice> })
  @ApiResponse({ status: 404, description: "ProductPrice not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the productprice to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProductPriceResponse<ProductPrice>> {
    try {
      const productprice = await this.service.findOne({ where: { id } });
      if (!productprice) {
        throw new NotFoundException(
          "ProductPrice no encontrado para el id solicitado"
        );
      }
      return productprice;
    } catch (error) {
