 ...
 * Licencia: MIT
 *
 * OnActiveStatusExpirableChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `active-status-expirable` (owner: catalog). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'active-status-expirable';
...
const TOPICS: string[] = [
  'active-status-expirable-created',
  'active-status-expirable-updated',
  'active-status-expirable-deleted',
];

@Injectable()
export class OnActiveStatusExpirableChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnActiveStatusExpirableChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnActiveStatusExpirableChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnActiveStatusExpirableChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onActiveStatusExpirableChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
