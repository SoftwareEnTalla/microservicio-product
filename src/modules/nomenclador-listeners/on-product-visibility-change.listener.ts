 ...
 * Licencia: MIT
 *
 * OnProductVisibilityChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `product-visibility` (owner: product). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'product-visibility';
...
const TOPICS: string[] = [
  'product-visibility-created',
  'product-visibility-updated',
  'product-visibility-deleted',
];

@Injectable()
export class OnProductVisibilityChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnProductVisibilityChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnProductVisibilityChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnProductVisibilityChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onProductVisibilityChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
