 ...
 * Licencia: MIT
 *
 * OnTargetEntityTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `target-entity-type` (owner: product). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'target-entity-type';
const OWNER_BC = 'product';
...
const TOPICS: string[] = [
  'target-entity-type-created',
  'target-entity-type-updated',
  'target-entity-type-deleted',
];

@Injectable()
export class OnTargetEntityTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnTargetEntityTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnTargetEntityTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnTargetEntityTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onTargetEntityTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
