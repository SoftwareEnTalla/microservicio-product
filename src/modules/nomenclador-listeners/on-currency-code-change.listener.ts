 ...
 * Licencia: MIT
 *
 * OnCurrencyCodeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `currency-code` (owner: catalog). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'currency-code';
const OWNER_BC = 'catalog';
const CROSS_CONTEXT = true;
const EVENT_NAMES: string[] = [
  'CurrencyCodeCreatedEvent',
  'CurrencyCodeUpdatedEvent',
  'CurrencyCodeDeletedEvent',
];
const TOPICS: string[] = [
...
@Injectable()
export class OnCurrencyCodeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnCurrencyCodeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnCurrencyCodeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnCurrencyCodeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onCurrencyCodeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
