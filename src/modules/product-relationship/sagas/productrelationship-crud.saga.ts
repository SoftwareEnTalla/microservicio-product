...
import {
  ProductRelationshipCreatedEvent,
  ProductRelationshipUpdatedEvent,
  ProductRelationshipDeletedEvent,

} from '../events/exporting.event';
import {
  SagaProductRelationshipFailedEvent
} from '../events/productrelationship-failed.event';
import {
...
export class ProductRelationshipCrudSaga {
  ...
  @Saga()
  onProductRelationshipCreated = ($events: Observable<ProductRelationshipCreatedEvent>) => {
    return $events.pipe(
  ...
  @Saga()
  onProductRelationshipUpdated = ($events: Observable<ProductRelationshipUpdatedEvent>) => {
    return $events.pipe(
  ...
  @Saga()
  onProductRelationshipDeleted = ($events: Observable<ProductRelationshipDeletedEvent>) => {
    return $events.pipe(
  ...
  private handleSagaError(error: Error, event: any) {
    ...
    this.eventBus.publish(new SagaProductRelationshipFailedEvent( error,event));
  }
}
