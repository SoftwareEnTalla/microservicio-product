...
@ApiTags("StockStatus Query")
@UseGuards(StockStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("stockstatuss/query")
export class StockStatusQueryController {
  #logger = new Logger(StockStatusQueryController.name);

  constructor(private readonly service: StockStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all stockstatus with optional pagination" })
  @ApiResponse({ status: 200, type: StockStatussResponse })
  @ApiQuery({ name: "options", required: false, type: StockStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<StockStatus>    
  ): Promise<StockStatussResponse<StockStatus>> {
    try {
     
      const stockstatuss = await this.service.findAll(options);
      logger.info("Retrieving all stockstatus");
      return stockstatuss;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find stockstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter stockstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: StockStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "StockStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find stockstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<StockStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all stockstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count stockstatuss with conditions" })
  @ApiResponse({ status: 200, type: StockStatussResponse<StockStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades StockStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one stockstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one stockstatus or return error" })
  @ApiResponse({ status: 200, type: StockStatusResponse<StockStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad StockStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get stockstatus by ID" })
  @ApiResponse({ status: 200, type: StockStatusResponse<StockStatus> })
  @ApiResponse({ status: 404, description: "StockStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the stockstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<StockStatusResponse<StockStatus>> {
    try {
      const stockstatus = await this.service.findOne({ where: { id } });
      if (!stockstatus) {
        throw new NotFoundException(
          "StockStatus no encontrado para el id solicitado"
        );
      }
      return stockstatus;
    } catch (error) {
