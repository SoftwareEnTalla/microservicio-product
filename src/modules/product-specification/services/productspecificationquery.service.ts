...
@Injectable()
export class ProductSpecificationQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(ProductSpecificationQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: ProductSpecificationQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${ProductSpecification.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const productspecifications = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de productspecifications obtenido con éxito",
        ...
        count: productspecifications.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<ProductSpecificationResponse<ProductSpecification>> {
    try {
      const productspecification = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "ProductSpecifications no encontrados para la propiedad y valor especificado"
        );
      // Devolver productspecification
      return {
        ok: true,
        message: "ProductSpecifications obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<ProductSpecificationResponse<ProductSpecification>> {
    try {
