...
import { ProductAttributeDto } from "../dtos/all-dto";
...
@ApiTags("ProductAttribute Query")
@Controller("productattributes/query")
export class ProductAttributeQueryController {
  #logger = new Logger(ProductAttributeQueryController.name);

  constructor(private readonly service: ProductAttributeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all productattribute with optional pagination" })
  @ApiResponse({ status: 200, type: ProductAttributesResponse })
  @ApiQuery({ name: "options", required: false, type: ProductAttributeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProductAttribute>    
  ): Promise<ProductAttributesResponse<ProductAttribute>> {
    try {
     
      const productattributes = await this.service.findAll(options);
      logger.info("Retrieving all productattribute");
  ...
  @ApiOperation({ summary: "Find productattribute by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter productattribute', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProductAttributesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProductAttribute no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find productattributes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProductAttribute>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all productattributes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count productattributes with conditions" })
  @ApiResponse({ status: 200, type: ProductAttributesResponse<ProductAttribute> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ProductAttributes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one productattribute with conditions" })
  ...
  @ApiOperation({ summary: "Find one productattribute or return error" })
  @ApiResponse({ status: 200, type: ProductAttributeResponse<ProductAttribute> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ProductAttribute no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the productattribute to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProductAttributeResponse<ProductAttribute>> {
    try {
      const productattribute = await this.service.findOne({ where: { id } });
      if (!productattribute) {
        throw new NotFoundException(
          "ProductAttribute no encontrado para el id solicitado"
        );
      }
