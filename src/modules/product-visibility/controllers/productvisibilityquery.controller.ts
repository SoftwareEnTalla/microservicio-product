...
@ApiTags("ProductVisibility Query")
@UseGuards(ProductVisibilityAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("productvisibilitys/query")
export class ProductVisibilityQueryController {
  #logger = new Logger(ProductVisibilityQueryController.name);

  constructor(private readonly service: ProductVisibilityQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all productvisibility with optional pagination" })
  @ApiResponse({ status: 200, type: ProductVisibilitysResponse })
  @ApiQuery({ name: "options", required: false, type: ProductVisibilityDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProductVisibility>    
  ): Promise<ProductVisibilitysResponse<ProductVisibility>> {
    try {
     
      const productvisibilitys = await this.service.findAll(options);
      logger.info("Retrieving all productvisibility");
  ...
  @ApiOperation({ summary: "Find productvisibility by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter productvisibility', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProductVisibilitysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProductVisibility no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find productvisibilitys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProductVisibility>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all productvisibilitys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count productvisibilitys with conditions" })
  @ApiResponse({ status: 200, type: ProductVisibilitysResponse<ProductVisibility> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one productvisibility with conditions" })
  ...
  @ApiOperation({ summary: "Find one productvisibility or return error" })
  @ApiResponse({ status: 200, type: ProductVisibilityResponse<ProductVisibility> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get productvisibility by ID" })
  @ApiResponse({ status: 200, type: ProductVisibilityResponse<ProductVisibility> })
  @ApiResponse({ status: 404, description: "ProductVisibility not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the productvisibility to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProductVisibilityResponse<ProductVisibility>> {
    try {
      const productvisibility = await this.service.findOne({ where: { id } });
