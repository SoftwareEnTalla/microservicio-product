...
import { RelationshipTypeDto } from "../dtos/all-dto";
...
@ApiTags("RelationshipType Query")
@UseGuards(RelationshipTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("relationshiptypes/query")
export class RelationshipTypeQueryController {
  #logger = new Logger(RelationshipTypeQueryController.name);

  constructor(private readonly service: RelationshipTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all relationshiptype with optional pagination" })
  @ApiResponse({ status: 200, type: RelationshipTypesResponse })
  @ApiQuery({ name: "options", required: false, type: RelationshipTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<RelationshipType>    
  ): Promise<RelationshipTypesResponse<RelationshipType>> {
    try {
     
      const relationshiptypes = await this.service.findAll(options);
      logger.info("Retrieving all relationshiptype");
  ...
  @ApiOperation({ summary: "Find relationshiptype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter relationshiptype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: RelationshipTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "RelationshipType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find relationshiptypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<RelationshipType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all relationshiptypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count relationshiptypes with conditions" })
  @ApiResponse({ status: 200, type: RelationshipTypesResponse<RelationshipType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one relationshiptype with conditions" })
  ...
  @ApiOperation({ summary: "Find one relationshiptype or return error" })
  @ApiResponse({ status: 200, type: RelationshipTypeResponse<RelationshipType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get relationshiptype by ID" })
  @ApiResponse({ status: 200, type: RelationshipTypeResponse<RelationshipType> })
  @ApiResponse({ status: 404, description: "RelationshipType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the relationshiptype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<RelationshipTypeResponse<RelationshipType>> {
    try {
      const relationshiptype = await this.service.findOne({ where: { id } });
      if (!relationshiptype) {
        throw new NotFoundException(
