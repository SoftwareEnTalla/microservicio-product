...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class RelationshipTypeQueryRepository {

    //Constructor del repositorio de datos: RelationshipTypeQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination RelationshipType on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[RelationshipType[], number]> {
      logger.info('Ready to findAndCount RelationshipType on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of RelationshipType on repository:',result);
      return result;
    }
