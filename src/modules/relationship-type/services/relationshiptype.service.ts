...
export class RelationshipTypeService {
  constructor(private readonly repository: RelationshipTypeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<RelationshipType[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<RelationshipType[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<RelationshipType[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<RelationshipType | null> {
  ...
  async update(id: string, partialEntity: Partial<RelationshipType>): Promise<RelationshipType | null> {
