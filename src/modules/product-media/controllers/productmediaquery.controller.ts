...
import { ProductMediaDto } from "../dtos/all-dto";
...
export class ProductMediaQueryController {
  #logger = new Logger(ProductMediaQueryController.name);

  constructor(private readonly service: ProductMediaQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all productmedia with optional pagination" })
  @ApiResponse({ status: 200, type: ProductMediasResponse })
  @ApiQuery({ name: "options", required: false, type: ProductMediaDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProductMedia>    
  ): Promise<ProductMediasResponse<ProductMedia>> {
    try {
     
      const productmedias = await this.service.findAll(options);
      logger.info("Retrieving all productmedia");
  ...
  @ApiParam({ name: 'field', required: true, description: 'Field to filter productmedia', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProductMediasResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProductMedia no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find productmedias with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProductMedia>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all productmedias" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count productmedias with conditions" })
  @ApiResponse({ status: 200, type: ProductMediasResponse<ProductMedia> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one productmedia with conditions" })
  ...
  @ApiOperation({ summary: "Find one productmedia or return error" })
  @ApiResponse({ status: 200, type: ProductMediaResponse<ProductMedia> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
