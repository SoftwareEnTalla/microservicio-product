...
@ApiTags("ProductStatus Query")
@UseGuards(ProductStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("productstatuss/query")
export class ProductStatusQueryController {
  #logger = new Logger(ProductStatusQueryController.name);

  constructor(private readonly service: ProductStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all productstatus with optional pagination" })
  @ApiResponse({ status: 200, type: ProductStatussResponse })
  @ApiQuery({ name: "options", required: false, type: ProductStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ProductStatus>    
  ): Promise<ProductStatussResponse<ProductStatus>> {
    try {
     
      const productstatuss = await this.service.findAll(options);
      logger.info("Retrieving all productstatus");
  ...
  @ApiOperation({ summary: "Find productstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter productstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProductStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ProductStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find productstatuss with pagination" })
  @ApiResponse({ status: 200, type: ProductStatussResponse<ProductStatus> })
  @ApiQuery({ name: "options", required: false, type: ProductStatusDto }) // Ajustar según el tipo real
  @ApiQuery({ name: "page", required: false, type: Number })
  @ApiQuery({ name: "size", required: false, type: Number })
  @ApiQuery({ name: "sort", required: false, type: String })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ProductStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all productstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count productstatuss with conditions" })
  @ApiResponse({ status: 200, type: ProductStatussResponse<ProductStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ProductStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one productstatus with conditions" })
  @ApiResponse({ status: 200, type: ProductStatusResponse<ProductStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  @ApiOperation({ summary: "Find one productstatus or return error" })
  @ApiResponse({ status: 200, type: ProductStatusResponse<ProductStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ProductStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get productstatus by ID" })
  @ApiResponse({ status: 200, type: ProductStatusResponse<ProductStatus> })
  @ApiResponse({ status: 404, description: "ProductStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the productstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProductStatusResponse<ProductStatus>> {
    try {
      const productstatus = await this.service.findOne({ where: { id } });
      if (!productstatus) {
        throw new NotFoundException(
          "ProductStatus no encontrado para el id solicitado"
        );
      }
