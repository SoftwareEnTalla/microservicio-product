...
export class ProductQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(ProductQueryService.name);
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${Product.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      ...
      return {
        ok: true,
        message: "Listado de products obtenido con éxito",
        data: products,
  ...
  async findById(id: string): Promise<ProductResponse<Product>> {
    try {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "Products no encontrados para la propiedad y valor especificado"
        );
      // Devolver product
      return {
        ok: true,
